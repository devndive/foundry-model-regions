	...
	for (const { region, models: regionModels } of snapshots) {
		for (const entry of regionModels) {
			...
			modelsById.set(id, {
				...
				isDefaultVersion:
					m.isDefaultVersion ?? existing?.isDefaultVersion ?? false,
				capabilities:
				...
				deprecation:
					modelDeprecation(m.deprecation) ?? existing?.deprecation ?? null,
			});
			...
			for (const sku of m.skus ?? []) {
				...
				availability.push({
					...
					region,
					sku: sku.name,
					deprecationDate: sku.deprecationDate
						? String(sku.deprecationDate)
						: null,
				});
			}
		}
